...
import { ShoppingCart, User, Mail, Lock, Eye, EyeOff, IdCard, Phone, ShieldCheck, RefreshCw, Send, Check } from 'lucide-react';
...
export default function SignUpPage({ onLogin }: SignUpPageProps) {
  ...
  const handleSendOTP = async () => {
    ...
    setErrors({});
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/auth/send-registration-otp`, {
        method: 'POST',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        // Auto-login: store credentials exactly like CustomerLogin does
        apiService.saveCustomerAuthData(token, customer);
        onLogin('customer');
